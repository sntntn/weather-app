...
TEST_CASE("DetailedWeatherAPI testing"){
    SECTION("DetailedWeatherData sent with dataFetched signal contains "
        ...
        // Act
        QObject::connect(&api, &DetailedWeatherAPI::dataFetched, [&location2](const QSharedPointer<DetailedWeatherData> &data){
    ...
    SECTION("DetailedWeatherAPI returns errorOccurred signal if the coordinates are incorrect"){
        ...
        // Act
        QObject::connect(&api, &DetailedWeatherAPI::errorOccurred, [&errorOccurred](){
            errorOccurred = true;
        });
