...
SettingsDialog::SettingsDialog(QWidget *parent)
	: QDialog(parent), settings(Settings::instance()), mainLayout(new QVBoxLayout(this)),
	  locationSwitch(new QCheckBox("Share Location", this)), temperatureUnit(new QComboBox(this)),
	  windSpeedUnit(new QComboBox(this)), precipitationUnit(new QComboBox(this)), listWidget(new WidgetsManager(this)),
	  buttonLayout(new QHBoxLayout()), save(new QPushButton("Save")), cancel(new QPushButton("Cancel")),
	  widgetOrder(settings.savedLocations()), trashCan("../Resources/trashCan/redTrash.png"),
	  orderCan("../Resources/order/order.png"), trashIcon(trashCan), orderIcon(orderCan)
{
  ...
  for (int i = 0; i < precKeyCount; ++i)
  {
	...
	precipitationUnit->addItem(settings.precipitationUnitsNames(precipUnit), QVariant::fromValue(precipUnit));
  }
  ...
  for (const auto &location : settings.savedLocations())
  {
	...
	connect(locationNameEdit, &QLineEdit::returnPressed, [locationNameEdit]() { locationNameEdit->clearFocus(); });
  ...
  connect(listWidget, &WidgetsManager::itemsRearranged, this,
		  [this]()
		  {
			widgetOrder.clear();
  ...
  connect(this, &SettingsDialog::settingsChanged, qobject_cast<MainWindow *>(this->parent()->parent()->parent()),
...
void SettingsDialog::changeSettings()
{
  Settings::TemperatureUnit selectedTempUnit =
	  static_cast<Settings::TemperatureUnit>(temperatureUnit->itemData(temperatureUnit->currentIndex()).toInt());
  Settings::WindSpeedUnit selectedWindUnit =
	  static_cast<Settings::WindSpeedUnit>(windSpeedUnit->itemData(windSpeedUnit->currentIndex()).toInt());
  Settings::PrecipitationUnit selectedPrecUnit =
	  static_cast<Settings::PrecipitationUnit>(precipitationUnit->itemData(precipitationUnit->currentIndex()).toInt());
