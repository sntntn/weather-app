...
singleWidgetItem::singleWidgetItem(const QString &iconPath, const QString &infoName, QWidget *parent)
	: QWidget(parent), mainLayout(new QVBoxLayout(this)), lowerLayout(new QHBoxLayout()), infoIcon(QPixmap(iconPath)),
	  infoIconLabel(new QLabel(this)), infoLabel(new QLabel(infoName)), info(new QLabel(this))
{
  mainLayout->addWidget(infoLabel);
  infoIconLabel->setPixmap(infoIcon.scaled(iconWidth, iconHeight, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
LocationInfoWidget::LocationInfoWidget(QWidget *parent)
	: QWidget(parent), mainLayout(new QVBoxLayout(this)), cityLabel(new QLabel(this)), countryLabel(new QLabel(this))
{
...
void LocationInfoWidget::updateData(const QGeoCoordinate &newCoordinates, const QString &cityName,
...
BasicInfoWidget::BasicInfoWidget(QWidget *parent)
	: QWidget(parent), basicInfoLayout(new QHBoxLayout()), leftLayout(new QVBoxLayout()),
	  rightLayout(new QVBoxLayout()), dateTimeLayout(new QVBoxLayout()), iconLabel(new QLabel(this)),
	  weatherDescriptionLabel(new QLabel(this)), todayLabel(QLabel("Today,")), dateLabel(new QLabel(this)),
	  timeLabel(new QLabel(this)), temperatureLabel(new QLabel(this)), feelsLikeLabel(new QLabel(this))
{
...
void BasicInfoWidget::updateData(const int weatherCode, const bool isDay, const QTimeZone &timezone,
								 const int temperature, const int apparentTemperature)
{
...
// --------------------------------------------------------------
MinMaxTempWidget::MinMaxTempWidget(QWidget *parent) : QWidget{parent}, mainLayout(new QHBoxLayout(this))
{
...
MinMaxTempWidget::MinMaxTempWidgetItem::MinMaxTempWidgetItem(const QString &iconPath, const QString &infoName,
															 QWidget *parent)
	: QWidget(parent), mainLayout(new QHBoxLayout(this)), infoIcon(QPixmap(iconPath)), infoIconLabel(new QLabel(this)),
	  infoLabel(new QLabel(infoName)), info(new QLabel(this))
{
  infoIconLabel->setPixmap(infoIcon.scaled(iconWidth, iconHeight, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
// Sunrize / sunset time
// --------------------------------------------------------------
SunWidget::SunWidget(QWidget *parent) : QWidget(parent), mainLayout(new QHBoxLayout(this))
{
...
SunWidget::SunWidgetItem::SunWidgetItem(const QString &iconPath, const QString &infoName, QWidget *parent)
	: QWidget(parent), mainLayout(new QHBoxLayout(this)), timeLayout(new QVBoxLayout()), infoIcon(QPixmap(iconPath)),
	  infoIconLabel(new QLabel(this)), infoLabel(new QLabel(infoName)), info(new QLabel(this)),
...
void SunWidget::SunWidgetItem::updateData(const QTimeZone &timezone, const QVector<QString> &values)
{
  ...
  additionalinfo->setText("+" + QString::number(hours) + "h " + QString::number(minutes) + "m");
}
...
WindInfoWidget::WindInfoWidget(QWidget *parent)
	: QWidget(parent), mainLayout(new QHBoxLayout(this)), leftLayout(new QVBoxLayout()), rightLayout(new QVBoxLayout()),
	  windGustsLabel(new QLabel("Wind Gusts")), windGusts(new QLabel()), windSpeedLabel(new QLabel("Wind Speed")),
	  windSpeed(new QLabel()), compassLabel(new QLabel(this)),
	  initialCompassIcon(QPixmap("../Resources/wind/compass.png")), arrowIcon(QPixmap("../Resources/wind/arrow.png"))
{
...
void WindInfoWidget::updateData(const int windSpeedValue, const int windGustsValue, const int windDirectionValue)
{
  ...
  painter.drawPixmap(center.x() - arrowIcon.width() / 2, center.y() - arrowIcon.height() / 2, arrowIcon);
  painter.end();
...
HourlyWeatherWidget::HourlyWeatherWidget(QWidget *parent)
	: QWidget(parent), mainLayout(new QVBoxLayout(this)), hourlyLabel(new QLabel("      Hourly")),
	  hourlyWeatherArea(new QScrollArea(this)), hourlyWeatherContents(new QWidget(this)), itemsLayout(new QHBoxLayout())
{
									 ...
									 const QVector<bool> &isDays, const QVector<QString> &timeStamps)
{
  for (int i = 0; i < hoursPerDay; ++i)
  {
	...
	widget->updateData(temperatures[i], weatherCodes[i], isDays[i], timeStamps[i]);
  }
}

HourlyWeatherWidget::HourlyWidgetItem::HourlyWidgetItem(QWidget *parent)
	: QWidget(parent), hourLayout(new QVBoxLayout(this)), hourLabel(new QLabel()),
	  hourWeatherIconLabel(new QLabel(this)), hourTempLabel(new QLabel(this))
{
...
void HourlyWeatherWidget::HourlyWidgetItem::updateData(const int tempText, const int weatherCode, const bool isDay,
													   const QString &timeStamp)
{
  hourLabel->setText(timeStamp);
  hourWeatherIcon.load(Settings::instance().weatherCodeToIcon(weatherCode, isDay));
  hourWeatherIconLabel->setPixmap(
	  hourWeatherIcon.scaled(iconWidth, iconHeight, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
DailyWeatherWidget::DailyWeatherWidget(QWidget *parent)
	: QWidget{parent}, mainLayout(new QGridLayout(this)), dailyLabel(new QLabel("7-DAY FORECAST"))
{
  ...
  connect(this, &DailyWeatherWidget::showTemperatureGraph, this, &DailyWeatherWidget::onShowTemperatureGraph);
}
...
DailyWeatherWidget::DailyWidgetItem::DailyWidgetItem(QWidget *parent)
	: QWidget(parent), dailyLayout(new QHBoxLayout(this)), dayNameLabel(new QLabel()),
	  dayWeatherIconLabel(new QLabel(this)), dailyminTempLabel(new QLabel(this)),
	  temperatureIcon(QPixmap("../Resources/temperature/temperatureBlack.png")), temperatureIconLabel(new QLabel()),
	  dailymaxTempLabel(new QLabel(this))
{
  ...
  temperatureIconLabel->setPixmap(
	  temperatureIcon.scaled(iconWidth, iconHeight, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
void DailyWeatherWidget::DailyWidgetItem::updateData(const QString &dayName, const int weatherCode, const int minTemp,
													 const int maxTemp, const QVector<int> &temperatures)
{
  dayNameLabel->setText(dayName);
  dayWeatherIcon.load(Settings::instance().weatherCodeToIcon(weatherCode, true));
  dayWeatherIconLabel->setPixmap(
	  dayWeatherIcon.scaled(iconWidth, iconHeight, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
void DailyWeatherWidget::onShowTemperatureGraph(const QVector<int> &temperatures, const QString &dayNameString)
{
