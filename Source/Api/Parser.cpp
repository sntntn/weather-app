    ...
    -> WeatherData *
{
  try
  {
	...
	return new WeatherData(geoLocation, temperature, maxTemperature, minTemperature, weatherCode, isDay, timeZone);
  }
    ...
    -> QSharedPointer<DetailedWeatherData>
{
  try
  {
	...
	std::transform(hourlyTempJ.begin(), hourlyTempJ.end(), std::back_inserter(hourlyTemperatures),
				   [](const QJsonValue &value) { return static_cast<int>(qRound(value.toDouble())); });
	...
	for (int i = 0; i < 7; i++)
	{
	  ...
	  QString dayName =
		  QDate::fromString(static_cast<QString>(weeklyDayJ[i].toString()), "yyyy-MM-dd").toString("dddd");
	...
	QSharedPointer<DetailedWeatherData> data(new DetailedWeatherData(
		geoLocation, temperature, weatherCode, isDay, timeZone, windSpeed, windGusts, windDirection,
		apparentTemperature, precipitation, snowDepth, uvIndex, humidity, visibility, pressure, ht, hourlyTemperatures,
		hc, hd, hts, weeklyMaxTemp, weeklyMinTemp, weeklyCode, weeklySunrise, weeklySunset, weeklyDayName));
	return data;
  }
