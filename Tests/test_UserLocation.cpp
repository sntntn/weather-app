...
TEST_CASE("UserLocation signals")
{
		  ...
		  " is emitted when getLocation function is called")
  {
	...
	// Act
	QObject::connect(&userLocation, &UserLocation::userLocationFetched, [&fetched]() { fetched = true; });

	QObject::connect(&userLocation, &UserLocation::userLocationError, [&error]() { error = true; });
